...
def file():
    data = datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S")  # Corrected format
    file_dir = "Data/Sys_monitor"
    if not os.path.exists(file_dir):
        os.makedirs(file_dir)
    filename = f"system_monitoring_data_{data}.txt"  # No spaces or invalid characters
    file_path = os.path.join(file_dir, filename)  # OS-independent path
    return file_path
...
def monitor_system():
    while True:
        ...
        with open(file_path, 'a') as file:
                    ...
                    file.write(tabulate(data, headers=["Metric", "Value"], tablefmt="grid") + "\n")

                    file.write("\nTop Running Processes:\n")
                    file.write(tabulate(processes, headers=["PID", "Process Name", "CPU (%)", "Memory (%)"], tablefmt="grid") + "\n")
